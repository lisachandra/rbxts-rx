...
type MockMethod<T> = (this: void, object: InferThis<T>, ...args: Parameters<OmitThisParameter<T>>) => ReturnType<T>;
...
type MockInstanceDeep<T extends object, Original extends object> = MockInstance<Original> & {
  [K in keyof T]: T[K] extends object ? ReturnType<typeof TestUtils.createMockInstance<T[K]>> & T[K] : T[K];
};
...
type MockOnRuntime<T extends object> = MockInstanceDeep<jest.MockedObjectDeep<MockedObjectWithMethods<T>>, T> & {
  /**
   ...
   *   value or not.
   */
  __mockValue__: <K extends keyof MockOnRuntime<T>>(this: MockOnRuntime<T>, key: K, value: MockOnRuntime<T>[K]) => any;
};
...
declare const TestUtils: {
   ...
   */
  mockOnRuntime: <T extends object>(jestModule: typeof jest, mockInstance: MockInstance<T> & Writable<Partial<T>>) => MockOnRuntime<T>;
