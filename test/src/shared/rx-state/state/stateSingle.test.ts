...
describe('stateSingle', () => {
  describe('observable', () => {
    ...
    it('cleans up the state when an error happens with reentrant subscribers', (_, done) => {
      let nexts: number[] = [];
    ...
    it('errors when the source stream completes without having emitted', async () => {
      ...
      await expect(firstValueFrom(shared)).rejects.toThrowError(EmptyObservableError);
    });
  ...
  describe('getValue', () => {
    describe('without default value', () => {
      ...
      it('always returns the same promise while the value is not emitted', () => {
        ...
        expect(initialPromise).never.toBe(sourceState.getValue());
        source.next(1);
