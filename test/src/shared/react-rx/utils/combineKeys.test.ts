...
import { concat, NEVER, Observable, of } from '@rbxts/rx';
import { map, scan } from '@rbxts/rx/out/operators';
import { TestScheduler } from '@rbxts/rx/out/testing';
import { combineKeys, KeyChanges } from '@rbxts/react-rx/out/utils';
...
describe('combineKeys', () => {
  it('emits a map with the latest value of the stream of each key', () => {
    scheduler().run(({ expectObservable, cold }) => {
      const keys = cold('  ab---cd---').pipe(scan((acc, v) => [...acc, v], [] as Array<string>));
      const a = cold('     --1---2---');
      const b = cold('      ---------');
      const c = cold('          1----');
      const d = cold('           9---');
      ...
      const result = combineKeys(keys, (v): Observable<string> => innerStreams[v]).pipe(
        map((x) => {
          const acc = table.clone(x);
          acc.delete('changes');
          return acc;
        })
      );
  ...
  it('completes when the key stream completes', () => {
    scheduler().run(({ expectObservable, cold }) => {
      const keys = cold('  a-b---|').pipe(scan((acc, v) => [...acc, v], [] as Array<string>));
      const a = cold('     -1-----');
      const b = cold('       -1---');
      const expectedStr = '-e-f--|';
  ...
  describe('change set', () => {
    it('contains all of the keys initially present in the stream', () => {
      scheduler().run(({ expectObservable }) => {
        ...
        const result = combineKeys(keys, (v) => of(v)).pipe(map((x) => Object.keys(x.changes)));
    ...
    it('only contains those values that have changed from the previous emission', () => {
      scheduler().run(({ expectObservable, cold }) => {
        ...
        const result = combineKeys(keys, (v) => streams[v]).pipe(map((x) => Object.keys(x.changes)));
