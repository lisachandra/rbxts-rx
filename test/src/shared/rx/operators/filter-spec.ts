...
describe('filter', () => {
  ...
  it('should support type guards without breaking previous behavior', () => {
    ...
    // type guards with primitive types
    {
      ...
      xs.pipe(filter(isString)).subscribe((s) => s.size()); // s is string

      // In contrast, this type of regular boolean predicate still maintains the original type
      xs.pipe(filter((x) => typeIs(x, 'number'))).subscribe((x) => x); // x is still string | number
      xs.pipe(filter((x, i) => typeIs(x, 'number') && x > i)).subscribe((x) => x); // x is still string | number
    }
