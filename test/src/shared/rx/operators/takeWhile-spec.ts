...
describe('takeWhile', () => {
  ...
  it('should support type guards without breaking previous behavior', () => {
    ...
    // type guards with primitive types
    {
      ...
      xs.pipe(takeWhile((x) => typeIs(x, 'number'))).subscribe((x) => x); // x is still string | number
      xs.pipe(takeWhile((x, i) => typeIs(x, 'number') && x > i)).subscribe((x) => x); // x is still string | number
    }
  });
