...
import { zipWith, mergeMap } from '@rbxts/rx/out/operators';
import { queueScheduler, of, scheduled } from '@rbxts/rx';
...
describe('zipWith', () => {
  ...
  it('should work with error and error', () => {
    rxTestScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      expectObservable(a.pipe(zipWith(b))).toBe(expected, undefined, 'too bad');
      expectSubscriptions(a.subscriptions).toBe(asubs);
      expectSubscriptions(b.subscriptions).toBe(bsubs);
    });
  });
