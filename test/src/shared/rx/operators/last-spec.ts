...
describe('last', () => {
  ...
  it('should error on nothing sent but completed', () => {
    testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      expectObservable(e1.pipe(last())).toBe(expected, undefined, new EmptyError());
      expectSubscriptions(e1.subscriptions).toBe(e1subs);
    });
  });
  ...
  it('should return a default value if no element found', () => {
    testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
      ...
      expectObservable(e1.pipe(last(undefined, 'a'))).toBe(expected);
      expectSubscriptions(e1.subscriptions).toBe(e1subs);
    });
  });
  ...
  it('should not return default value if an element is found', () => {
    testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
      ...
      expectObservable(e1.pipe(last(undefined, 'x'))).toBe(expected);
      expectSubscriptions(e1.subscriptions).toBe(e1subs);
    });
  });
  ...
  it('should support type guards without breaking previous behavior', () => {
    ...
    // type guards with primitive types
    {
      ...
      xs.pipe(last((x) => typeIs(x, 'string'))).subscribe((x) => x); // x is still string | number
    }
