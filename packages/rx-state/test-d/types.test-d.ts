...
import { EMPTY, filter, from, map, Observable, OperatorFunction, startWith, switchMap } from '@rbxts/rx';
...
// StateObservable
{
  ...
  expectAssignable<StateObservable<1 | 2>>(null as any as DefaultedStateObservable<1 | 2>);
  ...
  expectNotAssignable<WithDefaultOperator<any, any>>(startWith<1 | 2>(undefined));
...
// state
{
  ...
  expectType<Observable<never>>(string$.pipe(switchMap((v) => optionalFactory$(v))));
  expectType<Observable<never>>(string$.pipe(switchMap((v) => optionalFactory$(v, Number(v)))));
