import type { Observable, OperatorFunction, UnaryFunction } from '@rbxts/rx';
import type { Error } from '@rbxts/luau-polyfill';

/// Effects
export declare function sinkSuspense<T>(): (source$: Observable<T>) => Observable<Exclude<T, SUSPENSE>>;

export function liftSuspense<T>(): (source$: Observable<T>) => Observable<T | SUSPENSE>;
...
export interface WithDefaultOperator<T, D> extends UnaryFunction<Observable<T>, DefaultedStateObservable<D>> {}
export declare function withDefault<T, D>(defaultValue: D): (source$: Observable<T>) => DefaultedStateObservable<T | D>;
...
type AddStopArg<A extends Array<any>> = number extends ReturnType<A['size']> ? A : [...args: A, _stop?: undefined];
