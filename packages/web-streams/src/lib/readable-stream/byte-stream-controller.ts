...
import { IsReadableStreamLocked, type ReadableByteStream, ReadableStreamClose, ReadableStreamError } from '../readable-stream';
...
function ReadableByteStreamControllerRespondInClosedState(controller: ReadableByteStreamController, firstDescriptor: PullIntoDescriptor) {
...
export function ReadableByteStreamControllerEnqueue(controller: ReadableByteStreamController, chunk: NonShared<ArrayBufferView>) {
  const stream = controller._controlledReadableByteStream;
...
export function ReadableByteStreamControllerRespondWithNewView(controller: ReadableByteStreamController, view: NonShared<ArrayBufferView>) {
  assert(controller._pendingPullIntos.length > 0);
