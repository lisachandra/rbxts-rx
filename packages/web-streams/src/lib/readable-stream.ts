...
import { promiseRejectedWith, promiseResolvedWith, setPromiseIsHandledToTrue, transformPromiseWith } from './helpers/webidl';
...
import type { ReadableStreamBYOBReaderReadOptions, ReadableStreamGetReaderOptions } from './readable-stream/reader-options';
...
export class ReadableStream<R = any> implements AsyncIterable<R> {
  ...
  constructor(
    ...
    if (underlyingSource.type === 'bytes') {
      ...
      SetUpReadableByteStreamControllerFromUnderlyingSource(this as unknown as ReadableByteStream, underlyingSource, highWaterMark);
    } else {
   ...
   */
  pipeThrough<RS extends ReadableStream>(transform: { readable: RS; writable: WritableStream<R> }, options?: StreamPipeOptions): RS;
  ...
  pipeTo(destination: WritableStream<R> | null | undefined, rawOptions: StreamPipeOptions | null | undefined = {}): Promise<void> {
    if (!IsReadableStream(this)) {
...
export { ReadableStreamDefaultReader, ReadableStreamBYOBReader };

// Controllers

export { ReadableStreamDefaultController, ReadableStreamBYOBRequest, ReadableByteStreamController };
