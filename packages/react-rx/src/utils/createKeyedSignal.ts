import { GroupedObservable, Observable, Observer } from '@rbxts/rx';
 ...
 * 2. The emitter function.
 */
export function createKeyedSignal<K, T>(): [(key: K) => GroupedObservable<K, T>, (key: K, value: T) => void];
 ...
 * 2. The emitter function.
 */
export function createKeyedSignal<K, T>(keySelector: (signal: T) => K): [(key: K) => GroupedObservable<K, T>, (signal: T) => void];
...
export function createKeyedSignal<K, T, A extends any[]>(
  ...
  return [
    (key: K) => {
      const res = new Observable<T>(function (observer) {
