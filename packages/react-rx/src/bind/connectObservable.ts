...
import { state } from '@rbxts/rx-state';
 ...
 * for the first value.
 */
export default function connectObservable<T>(observable: Observable<T>, defaultValue: T) {
  const sharedObservable = defaultValue === EMPTY_VALUE ? state(observable) : state(observable, defaultValue);
