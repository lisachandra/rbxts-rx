...
import { state, StateObservable, SUSPENSE } from '@rbxts/rx-state';
...
export default function connectFactoryObservable<A extends [], O>(
  ...
  const args: [(...args: A) => Observable<O>] | [(...args: A) => Observable<O>, O | ((...args: A) => O)] =
    defaultValue === EMPTY_VALUE ? [getObservable] : [getObservable, defaultValue];

  const obs = state(...(args as [(...args: A) => Observable<O>]));
  return [(...input: A) => useStateObservable(obs(...(input as never))), obs as any];
}
