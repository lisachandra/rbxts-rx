...
import { StateObservable, DefaultedStateObservable, SUSPENSE } from '@rbxts/rx-state';
 ...
 * for the first value.
 */
export function bind<T>(observable: Observable<T>): [() => Exclude<T, typeof SUSPENSE>, StateObservable<T>];
 ...
 * there are no subscribers to that observable.
 */
export function bind<T>(observable: Observable<T>, defaultValue: T): [() => Exclude<T, typeof SUSPENSE>, DefaultedStateObservable<T>];
...
): [(...args: AddStopArg<A>) => Exclude<O, typeof SUSPENSE>, (...args: AddStopArg<A>) => StateObservable<O>];
...
): [(...args: AddStopArg<A>) => Exclude<O, typeof SUSPENSE>, (...args: AddStopArg<A>) => DefaultedStateObservable<O>];

export function bind(observable: unknown, defaultValue?: unknown) {
  return (typeIs(observable, 'function') ? (connectFactoryObservable as Callback) : connectObservable)(
    observable,
    defaultValue !== undefined ? defaultValue : EMPTY_VALUE
  );
}
