...
it('should handle Boolean constructor properly', () => {
  const a = of(0 as const, -0 as const, undefined, undefined, false as const, '' as const, 0n as const).pipe(last(Boolean)); // $ExpectType Observable<never>
  const b = of(0 as const, -0 as const, undefined, undefined, false as const, '' as const, 0n as const).pipe(last(Boolean, 'test' as const)); // $ExpectType Observable<"test">
  const c = of(0 as const, -0 as const, undefined, 'hi' as const, undefined, false as const, '' as const, 0n as const).pipe(last(Boolean)); // $ExpectType Observable<"hi">
  const d = of(0 as const, -0 as const, undefined, 'hi' as const, undefined, false as const, '' as const, 0n as const).pipe(
    last(Boolean, 'test' as const)
  ); // $ExpectType Observable<"test" | "hi">
});
