...
it('should infer correctly', () => {
  const o = of(1, 2, 3).pipe(bufferToggle(of('a', 'b', 'c'), value => of(DateTime.now()))); // $ExpectType Observable<number[]>
});

it('should support Promises', () => {
  const promise = Promise.resolve('a');
  const o = of(1, 2, 3).pipe(bufferToggle(promise, value => of(DateTime.now()))); // $ExpectType Observable<number[]>
...
it('should support NEVER', () => {
  const o = of(1, 2, 3).pipe(bufferToggle(NEVER, value => of(DateTime.now()))); // $ExpectType Observable<number[]>
